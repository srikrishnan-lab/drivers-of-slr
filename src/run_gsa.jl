...
my_gmslr = model_ensemble(num_samples=n_samples, calibrated_params=A)
...
function f(M; A=A, B=B, year=2100)
...
function f(M_in; A_in=A_base, B_in=B_base, A_out=A_gmslr, B_out=B_gmslr, yr=2100) # takes in M_in, which is a matrix of parameters (same size as A_in and B_in)

...
for year in years
    plt = scatter(first_order.parameter, first_order[:,"$year"], yerror = first_CI[:,"$year"], legend=:false,
            title = "$year", xlabel="Parameter", ylabel="Sensitivity", xticks=:all, xrotation=45, 
            markersize=3, tickfontsize=6, bottom_margin=7mm, left_margin=5mm)
    push!(all_plts, plt)
end
...
for year in years
    plt = scatter(total_order.parameter, total_order[:,"$year"], yerror = total_CI[:,"$year"], legend=:false,
            title = "$year", xlabel="Parameter", ylabel="Sensitivity", xticks=:all, xrotation=45,
            markersize=3, tickfontsize=6, bottom_margin=7mm, left_margin=5mm)
    push!(all_plts, plt)
end

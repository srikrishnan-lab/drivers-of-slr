#= this script was used to determine if the inputs of n2o_concentration, aerosol_forcing, and other_forcing needed to
be updated to the closest RCP scenario (whether or not it would make a difference in final radiative forcing). 
Based on these plots, the verdict is that it DOES make a difference, resulting in the creation of the match_inputs()
function in "../src/functions.jl" =#
